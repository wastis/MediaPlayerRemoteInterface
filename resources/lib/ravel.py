...
class _ServerDispatchNode(_DispatchNode) :
    ...
    class _UserDataDict(dict) :
        ...
        def __delitem__(self, key) :
            ...
            if len(self) == 0 :
                bus = self._ravel_bus()
                assert bus  is not None, "parent Connection has gone"
...
class Connection(dbus.TaskKeeper) :
    ...
    def __new__(celf, connection, direct_connect) :
        ...
        if self  is None :
            ...
            if direct_connect :
                assert unique_name  is None, "connection already registered"
                self.bus_names_acquired = None
                self.bus_names_pending = None
            else :
                assert unique_name  is not None, "connection not yet registered"
    ...
    #begin __del__
        if self._server_dispatch  is not None :
            remove_listeners(self, True, self._server_dispatch, [])
        #end if
        if self._client_dispatch  is not None :
    ...
    def _bus_name_acquired(conn, msg, w_self) :
        ...
        if bus_name not in self.bus_names_acquired :
            ...
            if self._bus_name_acquired_action  is not None :
    ...
    def _bus_name_lost(conn, msg, w_self) :
        ...
        if bus_name in self.bus_names_acquired :
            ...
            if self._bus_name_lost_action  is not None :
    ...
    async def request_name_async(self, bus_name, flags, error = None, timeout = DBUS.TIMEOUT_USE_DEFAULT) :
        ...
        if not (is_acquired or is_pending) :
            ...
            if error  is not None and error.is_set or result != DBUS.REQUEST_NAME_REPLY_IN_QUEUE :
                self.bus_names_pending.discard(bus_name)
            #end if
        elif is_pending :
    ...
    def _get_dispatch_node(self, path, is_server, create_if) :
        ...
        if create_if :
            if is_server and self._server_dispatch  is None :
                self._server_dispatch = _ServerDispatchNode(self)
            elif not is_server and self._client_dispatch  is None :
        ...
        if level  is not None :
            ...
            while True :
                ...
                if component  is None :
                    break # found if level  is not None
    ...
    def register(self, path, fallback, interface, replace = True) :
        ...
        #end if
        if self._server_dispatch  is None :
            self._server_dispatch = _ServerDispatchNode(self)
        ...
        if level  is not None :
            ...
            while True :
                ...
                if (
                    ...
                    and
                        (level.interfaces[interface_name].fallback or component  is None)
                ) :
    ...
    def _get_all_my_props(self, message, path, interface_name) :
        ...
        while True :
            ...
            if "getter" in propentry :
                ...
                :
                    if getter._propgetter_info[keyword_keyword]  is not None :
                        value = value()
                        if value  is None :
                            raise ValueError \
                              (
    ...
    def prop_changed(self, path, interface, propname, proptype, propvalue) :
        ...
        " signal notification."
        assert (proptype  is not None) == (propvalue  is not None), \
            "either specify both of proptype and propvalue, or neither"
        if self.loop  is not None :
            queue_task = False
            if self._props_changed  is None :
                self._props_changed = {}
                queue_task = True
            #end if
    ...
    def _notify_objects_added(self) :
        ...
        if self._objects_added  is not None :
            notify_again = None
            if self.loop  is not None :
                now = self.loop.time()
            else :
    ...
    def _notify_objects_removed(self) :
        ...
        if self._objects_removed  is not None :
            notify_again = None
            if self.loop  is not None :
                now = self.loop.time()
            else :
        ...
        if level  is not None :
            ...
            while True :
                ...
                for interface_name, interface in level.interfaces.items() :
                    if component  is None or interface.fallback :
    ...
    def object_added(self, path, interfaces_and_props = None) :
        ...
        async def await_propvalues() :
            ...
            #end for
            if self._objects_added  is None : # might have happened in meantime
                self._objects_added = {}
                queue_task = True
            #end if
    ...
    #begin object_added
        ...
        #end if
        if self._objects_added  is None :
            self._objects_added = {}
            queue_task = True
        #end if
    ...
    def object_removed(self, path, interfaces = None) :
        ...
        #end if
        queue_task = False
        if self._objects_removed  is None :
        ...
        for interface in interfaces :
            if added_entry  is not None and interface in added_entry :
            ...
            if self._props_changed  is not None :
                ...
                if self._props_changed  is not None and props_key in self._props_changed :
        ...
        #end if
        if added_entry  is not None and len(added_entry) == 0 :
            del self._objects_added[path]
            if len(self._objects_added) == 0 :
                self._objects_added = None
            #end if
...
def _message_interface_dispatch(connection, message, w_bus) :
            ...
            if rulekey in signal_listeners and (len(path) == 0 or fallback) :
                ...
                for func in funcs :
                    try :
                        ...
                        :
                            if call_info[keyword_keyword]  is not None :
                                kwargs[call_info[keyword_keyword]] = value()
                            #end if
                        #end for
                        if call_info["args_keyword"]  is not None :
                            if call_info["arg_keys"]  is not None :
                                args = dict \
                                  (
                                    (key, value)
    ...
    def return_result_common(call_info, result) :
        ...
        if isinstance(result, dbus.Error) :
            ...
            if isinstance(result, dict) and call_info["result_keys"]  is not None :
                ...
                result = list(result)
            elif len(sig) == 0 and result  is None :
...
#begin _message_interface_dispatch
    ...
    if message.type in (DBUS.MESSAGE_TYPE_METHOD_CALL, DBUS.MESSAGE_TYPE_SIGNAL) :
        ...
        if not is_method and bus._client_dispatch  is not None :
                ...
                if is_method or not call_info["stub"] :
                    ...
                    try :
                        ...
                        if is_method :
                            ...
                            elif call_info["set_result_keyword"]  is not None :
                    ...
                    #end try
                    if result  is None :
                        if to_return_result  is not None or is_method :
                        ...
                        assert bus.loop  is not None, "no event loop to attach coroutine to"
                        if is_method :
                            # wait for result
                            async def await_result(coro) :
                                try :
                                    ...
                                    result = err.as_error()
                                #end try
                                if result  is None and to_return_result  is not None :
        ...
        :
            for fname in dir(celf) :
                ...
                if hasattr(func, info_type) :
                    ...
                    #end if
                    propentry = props[propname]
                    if propinfo["type"]  is not None :
                        if propentry["type"]  is not None :
                    ...
                    #end if
                    if meth_type == "getter" :
                        if propinfo["change_notification"]  is not None :
...
def method \
  (*,
    ...
    for cond, msg in \
        (
            (
                    (result_keyword  is not None or result_keys  is not None or result_attrs  is not None)
                and
                    not reply,
                "result_keyword, result_keys and result_attrs are"
                " meaningless if method does not reply",
            ),
            (arg_keys  is not None and arg_attrs  is not None, "specify arg_keys or arg_attrs, not both"),
            (arg_attrs  is not None and args_keyword  is None, "need args_keyword with arg_attrs"),
            (
                arg_keys  is not None and len(arg_keys) != len(in_signature),
                "number of arg_keys should match number of items in in_signature",
            ),
            (
                arg_attrs  is not None and len(arg_attrs) != len(in_signature),
                "number of arg_attrs should match number of items in in_signature",
            ),
            (
                set_result_keyword  is not None and result_keyword  is not None,
                "specify set_result_keyword or result_keyword, not both",
            ),
            (
                result_keys  is not None and result_attrs  is not None,
                "specify result_keys or result_attrs, not both",
            ),
            (
                result_attrs  is not None and result_keyword  is None,
                "need result_keyword with result_attrs",
            ),
            (
                result_keys  is not None and len(result_keys) != len(out_signature),
                "number of result_keys should match number of items in out_signature",
            ),
            (
                result_attrs  is not None and len(result_attrs) != len(out_signature),
...
def signal \
  (*,
    ...
    in_signature = dbus.parse_signature(in_signature)
    if arg_attrs  is not None and args_keyword  is None :
        raise ValueError("need args_keyword with arg_attrs")
    #end if
    if arg_keys  is not None and len(arg_keys) != len(in_signature) :
        raise ValueError("number of arg_keys should match number of items in in_signature")
    #end if
    if arg_attrs  is not None and len(arg_attrs) != len(in_signature) :
...
def propgetter \
  (*,
    ...
    def decorate(func) :
        ...
        if (
                change_notification  is not None
            and
...
def _append_args(message, call_info, args, kwargs) :
    ...
    if len(kwargs) != 0 :
        ...
        for arg_name in kwargs :
            ...
            if message_args[pos]  is not None :
                raise ValueError("duplicate value for arg %d" % pos)
            #end if
            message_args[pos] = kwargs[arg_name]
        #end for
    #end if
    missing = set(pos for pos in range(len(message_args)) if message_args[pos]  is None)
    if len(missing) != 0 :
        raise ValueError \
          (
...
def def_proxy_interface(kind, *, name, introspected, is_async) :
    ...
    class proxy(BusPeer.Object.ProxyInterface) :
        ...
        def __init__(self, *, parent, connection, dest, path, timeout = DBUS.TIMEOUT_USE_DEFAULT) :
            if is_async :
                assert connection.loop  is not None, "no event loop to attach coroutines to"
            #end if
            self._parent = parent
            ...
            def set_prop(self, value) :
                ...
                async def sendit() :
                    ...
                    if reply.type == DBUS.MESSAGE_TYPE_METHOD_RETURN :
                        ...
                        if self._parent._set_prop_failed  is None :
    ...
    class proxy_factory(BusPeer.RootProxy) :
        ...
        if is_async :
            async def set_prop_flush(self) :
                ...
                #end if
                if self._set_prop_failed  is not None :
                ...
                if len(set_prop_pending) != 0 :
                    ...
                    #end if
                    failed = list(e for f in done for e in (f.exception(),) if e  is not None)
                    if len(failed) > 1 :
                        raise RuntimeError \
                          (
    ...
    def introspect(self, message, bus, path) :
        ...
        while True :
            ...
            for entry in level.interfaces.values() :
                if component  is None or entry.fallback :
    ...
    #begin setprop
        ...
        if propname in props :
            ...
            if "setter" in propentry :
                ...
                try :
                    if propentry["type"]  is not None and propentry["type"] != dbus.parse_single_signature(proptype) :
                        raise ErrorReturn \
                          (
                            name = DBUS.ERROR_INVALID_ARGS,
                            message =
                    ...
                    :
                        if setter._propsetter_info[keyword_keyword]  is not None :
